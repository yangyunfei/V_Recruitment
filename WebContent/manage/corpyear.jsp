...
<script type="text/javascript">
	...
	$(function() {
		dataGrid = $('#dataGrid').datagrid({
			url : '${pageContext.request.contextPath}/yearDataController/dataGrid',
			fit : true,
			fitColumns : false,
			border : false,
			pagination : true,
			...
			},
			{
				...
				formatter : function(value, row, index) {
					...
					{
					str += $.formatString('<a  href="../fileController/downfile?id={0}&fileType=YD" target="_blank" title="文件">文件</a>', row.id);
					}
					return str;
				}
			} ] ],
	...
	function editAction(id) {
		...
		parent.$.modalDialog({
			...
			width : 800,
			height : 500,
			href : '${pageContext.request.contextPath}/yearDataController/editPage?id=' + id,
			buttons : [ {
				text : '保存',
				handler : function() {
	...
	function addAction() {
		parent.$.modalDialog({
			...
			width : 800,
			height : 400,
			href : '${pageContext.request.contextPath}/yearDataController/addPage'+'?r='+new Date().getTime(),
			buttons : [ {
				text : '添加',
				handler : function() {
					parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
		...
		parent.$.messager.confirm('询问', '您是否要删除当前数据？', function(b) {
			if (b) 
			{
				parent.$.messager.progress({
					...
					$.post('${pageContext.request.contextPath}/yearDataController/delete', {
						id : id
					}, function(result)
					 {
						if (result.success) 
		...
		parent.$.messager.confirm('询问', '您是否要上报当前数据?', function(b) {
			if (b) 
			{
				parent.$.messager.progress({
					...
					$.post('${pageContext.request.contextPath}/yearDataController/report', {
						id : id
					}, function(result)
					 {
						if (result.success) 
...
<body>
	...
	<div id="toolbar" style="display: none;">
		<c:if test="${1==1}">
			<a onclick="addAction();" href="javascript:void(0);" class="easyui-linkbutton" data-options="plain:true,iconCls:'pencil_add'" title="打开年度信息录入窗口"> 年度信息录入</a>
		</c:if>		 

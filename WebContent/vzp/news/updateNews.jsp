	...
	<div  style="margin: 2%">
		<form action="<%=basePath%>jf/newsController/addNews"
			...
			<table>
				<tr>
					<td>资讯标题:</td>
					<td><input type="text" name="news.title" value="${news.title }"></td>
				</tr>
				<tr>
					<td>资讯简介:</td>
					<td><textarea rows="3" cols="50" name="news.intro" value="${news.intro }"></textarea></td>
				</tr>
				<tr>
					<td>资讯图片:</td>
				...
				<tr>
					...
					<td><select name="news.publish">
							<option value="1"  <c:if test="${news.publish=1 }">selected="selected" </c:if>>全部</option>
							<option value="2" <c:if test="${news.publish=1 }">selected="selected" </c:if>>真有铺</option>
							<option value="3" <c:if test="${news.publish=1 }">selected="selected" </c:if>>专家团队</option>
					</select></td>
				</tr>
				<tr>
					<td>发布地点:</td>
			...
			<div>
				<script id="editor" type="text/plain"
					style="width:1024px;height:500px;" name="news.content" >
